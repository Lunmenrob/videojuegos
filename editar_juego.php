...
<body>
    ...
    <div class="edit-header">
        <h1 class="edit-title">Editar Juego</h1>
    </div>
...
<div class="form-group">
    ...
    <div class="edit-section" style="margin-top: 2rem; padding: 1.5rem; border: 2px solid #ff4444; border-radius: 12px; background: rgba(255, 68, 68, 0.1);">
        <h2 class="section-title" style="margin: 0 0 1rem 0; color: #ff4444;">Zona de Peligro</h2>
        <p style="margin: 0 0 1rem 0; color: #ff8888;">Esta acción es irreversible. Se eliminarán todos los datos del juego, incluyendo trofeos, DLCs y media.</p>
        <button type="button" onclick="deleteGame()" style="background: #ff4444; border: none; border-radius: 8px; color: #ffffff; padding: 0.75rem 1.5rem; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 0.5rem;">
            <i class="fas fa-trash"></i> Borrar Juego Completo
        </button>
    </div>
    </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Editar Juego - DOM cargado');
    ...
    console.log('game-id value:', document.getElementById('game-id').value);
    
    if (!gameId) {
        console.error('No se proporcionó ID del juego');
        alert('Error: No se proporcionó ID del juego');
        window.location.href = 'index.php';
        return;
    }
    
    document.getElementById('cancel-link').href = `detalles.php?id=${gameId}`;
    ...
    if (saveMediaBtn) {
        saveMediaBtn.addEventListener('click', function() {
            ...
            saveMediaGallery(gameId);
        });
    }
    
    // Cargar datos del juego y trofeos
    loadGameData(gameId);
    loadMediaItems(gameId);
    loadTrophies(gameId);
    loadDLCs(gameId);
});
...
function saveNewTrophy(gameId) {
    ...
    const trophyData = {
        ...
        instrucciones: document.getElementById('new-trophy-instrucciones').value,
...
function deleteTrophy(trophyId) {
    console.log('Eliminando trofeo:', trophyId);
...
function deleteDLC(dlcId) {
    console.log('Eliminando DLC:', dlcId);
...
// Función para borrar el juego completo
function deleteGame() {
    const gameId = new URLSearchParams(window.location.search).get('id');

    if (!confirm('¿Estás seguro de que quieres borrar este juego completo?\n\nEsta acción es IRREVERSIBLE y eliminará:\n- Todos los trofeos\n- Todos los DLCs\n- Todos los medios (imágenes y vídeos)\n- Todo el progreso\n\n¿Continuar?')) {
        return;
    }

    if (!confirm('ÚLTIMA ADVERTENCIA\n\nEsta acción NO se puede deshacer. Se borrará permanentemente todo el contenido del juego.\n\n¿Estás 100% seguro?')) {
        return;
    }

    console.log('Borrando juego:', gameId);

    fetch(`/Prácticas/videojuegos/api/games.php?id=${gameId}`, {
        method: 'DELETE'
    })
    .then(async res => {
        console.log('📡 Respuesta API:', res.status);
    ...
    .catch(err => {
        ...
        alert('Error borrando juego: ' + err.message);
    });
}
</script>
</body>
